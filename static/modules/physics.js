...
class SmokeCloud {
  ...
  add(x, y) {
    ...
    this.totalPixels.push(new SmokePixel(_x, _y, -0.05));
  }
}
...
export class SmokePlume {
  ...
  addSmoke(x, y) {
    ...
    this.newSmokePixels.push(new SmokePixel(_x, _y, this.velocityX, -0.05));

    smokePixels.push(new SmokePixel(_x, _y, this.velocityX, -0.05));
  }
  ...
  update(p5) {
	...
	totalSmokePixels = smokeClouds.length;
...
function addSmoke(x, y, vX) {
  ...
  smokePixels.push(new SmokePixel(_x, _y, vX));
}
...
export function updateAllSmokePixels() {
  ...
  for (let i = 0; i < l; i++) {
    ...
    if (smokePixels[i].spawn) {
      addSmoke(smokePixels[i].x, smokePixels[i].y, smokePixels[i].vX);
    }
  }

  for (let i = l - 1; i >= 0; i--) {
    if (smokePixels[i].dead || smokePixels[i].x < 0  // cull
        || smokePixels[i].y < 0) {
